...
struct DataPlotPrefs {
    ...
    std::string signalFilePrefix = "signalInputs/HHlnulnu_radHH_";
...
struct HistContainter {
    HistContainter() {
        bkg2D.resize(BKG_NOM+1); for(auto* b :bkg2D) b=0;
        bkg.resize(BKG_NOM+1); for(auto* b :bkg) b=0;
        toys2D.reserve(100);
    }
...
std::cout<<"sel: "<<s<<" --> bin: "<<iB<<std::endl;
...
void runPostFit(const std::string& inName, const std::string& outName, double fixR=0){
    ...
    fitter.addBkg(bkgSels[BKG_NOM],"_opt");
    ...
    for(const auto& l :lepCats) for(const auto& b :btagCats) for(const auto& h :hadCuts){
        ...
        const std::string wsName = "std_"+ l +"_"+b +"_"+h+"_13TeV";
        fitter.addCategory(l +"_"+b +"_"+h,wsName);
    }
...
void plotDileptonDataTests(int step = 0, int inreg = REG_SR,  const std::string limitBaseName = "/Users/brentstone/Dropbox/Physics/HHbbWW/limits/"){
    REGION reg = REGION(inreg);

    std:: string inName =  "bkgInputs" ;
    auto bcats = (reg == REG_QGCR ? qgBtagCats : btagCats);

    auto getSRCats = [&](REGION reg)->std::vector<std::string> {
        std::vector<std::string> sels;
        for(const auto& l :lepCats) for(const auto& b :bcats) for(const auto& h :hadCuts){
            if(l == lepCats[LEP_INCL]) continue;
            if(b == btagCats[BTAG_LMT]) continue;
            if(h != hadCuts[HAD_FULL]) continue;
    ...
    auto getSRCatTitles = [&](REGION reg)->std::vector<std::string> {
        std::vector<std::string> sels;
        for(const auto& l :lepCats) for(const auto& b :bcats) for(const auto& h :hadCuts){
            if(l == lepCats[LEP_INCL]) continue;
    ...
    auto srListTitles = getSRCatTitles(reg);
