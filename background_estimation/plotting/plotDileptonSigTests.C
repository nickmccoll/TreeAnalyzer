...
//                    }
                    std::string title = l.title+", "+ b.title +" category";
...
//                plot->topStyle.leftMarginSize=0.15;
                ...
                auto c = plot->draw(false,(name+"_"+l+"_"+h+"_sigeff") .c_str());
...
void plotDileptonSigTests(int cat = 0,int sig = RADION,  bool doCond = true, std::string outName = ""){
    ...
    case 0:
        ...
        plotYields(name,filename,"yield",{"sameFlav_L_full","sameFlav_M_full","sameFlav_T_full","oppFlav_L_full","oppFlav_M_full","oppFlav_T_full"});
        plotEfficiencies(name,filename,"yield" ,sig);
        break;
    case 1:
        if(outName.size()) outName += "_MJJ_fit1stIt";
        writeables = testSignal1DFits(name,filename,signalMassBins[sig],MOD_MJ,"MJJ_fit1stIt",{"inclFlav_LMT_full","inclFlav_L_full","inclFlav_M_full","inclFlav_T_full"});
        break;
    case 2:
        if(outName.size()) outName += "_MJJ_fit";
        writeables = testSignal1DFits(name,filename,signalMassBins[sig],MOD_MJ,"MJJ_fit",{"inclFlav_LMT_full","inclFlav_L_full","inclFlav_M_full","inclFlav_T_full"});
        break;
    case 3:
        if(outName.size()) outName += "_MVV_fit";
        if(doCond) writeables =  testSignal1DFits(name,filename,signalMassBins[sig],MOD_MR,"MVV_fit1stIt",{"oppFlav_LMT_full","sameFlav_LMT_full"});
    ...
    case 4:
        ...
        test2DFits(name,filename,signalMassBins[sig],"2D_fit",false,{"sameFlav_L_full","sameFlav_M_full","sameFlav_T_full","oppFlav_L_full","oppFlav_M_full","oppFlav_T_full"});
        //run below for an
