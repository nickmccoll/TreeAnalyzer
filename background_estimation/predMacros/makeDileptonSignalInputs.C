...
void makeSignal2DShapesSecondIteration(const std::string& name, const std::string& filename, const std::vector<int>& signalMassBins){
    ...
    for(const auto& l :lepCats) for(const auto& b :btagCats) for(const auto& h :hadCuts){
        ...
        const std::string catName = l +"_"+b +"_"+h;
        std::string mjjCatName = lepCats[LEP_INCL]+"_"+b+"_"+hadCuts[HAD_FULL];
        std::string mvvCatName = l+"_"+btagCats[BTAG_LMT]+"_"+hadCuts[HAD_FULL];
...
void makeDileptonSignalInputs(int step = 0,int sigToDo = RADION,std::string treeDir = "/Users/brentstone/Dropbox/Physics/HHbbWW/BETrees/"){
