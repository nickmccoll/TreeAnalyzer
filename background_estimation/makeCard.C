...
void go(const std::string& signalName, const std::string& filename) {
    ...
    for(const auto& l :lepSels) for(const auto& p :purSels) for(const auto& h :hadSels){
        ...
        card.w->var(MOD_MR.c_str())->setRange("coef",minInclHHMass,maxInclHHMass);
        card.w->var(MOD_MJ.c_str())->setRange("coef",30,210);
        ...
        if(signalName=="radHH"){
            card.add2DSignalParametricShape(signalName,MOD_MJ,MOD_MR, filename + "_"+signalName +"_"+l+"_"+p+"_"+h +"_2D_fit.json",
                    {{"CMS_scale_prunedj",1}},{{"CMS_res_prunedj",1}},{{"CMS_scale_j",1},{"CMS_scale_MET",1}},{{"CMS_res_j",1},{"CMS_res_MET",1}}, p == purSels[PUR_L],MOD_MS);
            card.addParametricYieldWithUncertainty(signalName,0,filename + "_"+signalName +"_"+l+"_"+p+"_"+h +"_yield.json",1,"CMS_tau21_PtDependence","log("+MOD_MS+"/600)",0.041,MOD_MS);
        ...
        card.addHistoShapeFromFile(bkgSels[BKG_QG],{MOD_MJ,MOD_MR}, filename + "_"+bkgSels[BKG_QG] +"_"+l+"_"+p+"_"+h +"_2D_template.root","histo",{qgSyst("PTX"),qgSyst("OPTX"),qgSyst("PTY"),qgSyst("OPTY")});
        card.addFixedYieldFromFile(bkgSels[BKG_QG],1,filename+"_"+bkgSels[BKG_QG]+"_distributions.root",bkgSels[BKG_QG]+"_"+l+"_"+p+"_"+h+"_"+hhMCS);
        //lost t/w bkg
        auto lostTWSyst =[&](const std::string& n)->StrStr{return StrStr(n,std::string("CMS_"+filename+"_"+bkgSels[BKG_LOSTTW]+"_"+n+"_"+l+"_"+p ));};
        card.addHistoShapeFromFile(bkgSels[BKG_LOSTTW],{MOD_MJ,MOD_MR}, filename + "_"+bkgSels[BKG_LOSTTW] +"_"+l+"_"+p+"_"+h +"_2D_template.root","histo",{lostTWSyst("PTX"),lostTWSyst("OPTX"),lostTWSyst("PTY"),lostTWSyst("OPTY")});
        card.addFixedYieldFromFile(bkgSels[BKG_LOSTTW],2,filename+"_"+bkgSels[BKG_LOSTTW]+"_distributions.root",bkgSels[BKG_LOSTTW]+"_"+l+"_"+p+"_"+h+"_"+hhMCS);

        //mW
        card.add1DBKGParametricShape(bkgSels[BKG_MW],MOD_MJ,filename+"_"+bkgSels[BKG_MW]+"_"+lepSels[LEP_EMU]+"_"+purSels[PUR_LMT]+"_"+hadSels[HAD_NONE]+"_fit.json",{{"CMS_scale_prunedj",1}},{{"CMS_res_prunedj",1}},MOD_MR,MOD_MJ);
        auto lostMWSyst =[&](const std::string& n)->StrStr{return StrStr(n,std::string("CMS_"+filename+"_"+bkgSels[BKG_MW]+"_"+n+"_"+l+"_"+p ));};
        card.addHistoShapeFromFile(bkgSels[BKG_MW],{MOD_MR}, filename + "_"+bkgSels[BKG_MW] +"_"+l+"_"+p+"_"+h +"_template.root","histo",{lostMWSyst("PT"),lostMWSyst("OPT")},false,0,MOD_MR);
        card.conditionalProduct(bkgSels[BKG_MW],bkgSels[BKG_MW] + "_"+MOD_MJ,MOD_MR,bkgSels[BKG_MW] + "_"+MOD_MR);
        card.addFixedYieldFromFile(bkgSels[BKG_MW],3,filename+"_"+bkgSels[BKG_MW]+"_distributions.root",bkgSels[BKG_MW]+"_"+l+"_"+p+"_"+h+"_"+hhMCS);

        //mT
        card.add1DBKGParametricShape(bkgSels[BKG_MT],MOD_MJ,filename+"_"+bkgSels[BKG_MT]+"_"+lepSels[LEP_EMU]+"_"+p+"_"+hadSels[HAD_NONE]+"_fit.json",{{"CMS_scale_prunedj",1}},{{"CMS_res_prunedj",1}},MOD_MR,MOD_MJ);
        auto lostMTSyst =[&](const std::string& n)->StrStr{return StrStr(n,std::string("CMS_"+filename+"_"+bkgSels[BKG_MT]+"_"+n+"_"+l+"_"+p ));};
        card.addHistoShapeFromFile(bkgSels[BKG_MT],{MOD_MR}, filename + "_"+bkgSels[BKG_MT] +"_"+l+"_"+p+"_"+h +"_template.root","histo",{lostMTSyst("PT"),lostMTSyst("OPT")},false,0,MOD_MR);
        card.conditionalProduct(bkgSels[BKG_MT],bkgSels[BKG_MT] + "_"+MOD_MJ,MOD_MR,bkgSels[BKG_MT]+ "_"+MOD_MR);
        card.addFixedYieldFromFile(bkgSels[BKG_MT],4,filename+"_"+bkgSels[BKG_MT]+"_distributions.root",bkgSels[BKG_MT]+"_"+l+"_"+p+"_"+h+"_"+hhMCS);

        //Data
//        card.importBinnedData(filename + "_data.root","data_"+l+"_"+p+"_"+h,{MOD_MJ,MOD_MR});
        card.importBinnedData(filename + "_pd.root","data_"+l+"_"+p+"_"+h+"_hbbMass_hhMass",{MOD_MJ,MOD_MR});
